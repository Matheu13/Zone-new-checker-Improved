...
- **SSRF protections** on all server-side outbound requests
...
- **Trusted proxies**: Vercel is detected automatically; other proxies require an explicit environment setting
...
- Credential-bearing redirects to another hostname are rejected
- Remote JSON and image responses have strict size limits

This application intentionally accepts only publicly routed portal URLs. Private LAN portals are rejected to prevent the deployed server from becoming an internal-network proxy.
