...
- Shared public-network URL validation for checker, playlist, Reddit, and image requests
- Redirect validation and protection against cross-host credential forwarding
- Bounded JSON, image, and request-body readers
- Security response headers and a least-privilege CI workflow
- Automated tests for network filtering, response limits, and verification fail-closed behavior

### Fixed

- Turnstile tokens could be submitted repeatedly after successful verification
- Reddit hostname suffix checks accepted lookalike domains
- Reddit short-link redirects were not constrained to Reddit domains
- Production verification could be bypassed when the cookie secret was missing
- Lint failures were hidden during production builds
