...
export async function POST(req: Request) {
  ...
  try {
    ...
    try {
      ...
      await assertSafePublicUrl(portalBase);
    ...
    for (const u of genreUrls) {
      const res = await fetchWithTimeout(u, { headers, timeoutMs: 20000, publicOnly: true });
      if (res.ok) {
    ...
    if (genreId) {
      ...
      const fetchPage = async (p: number): Promise<{ chans: StalkerChannel[]; hasMore: boolean; totalItems?: number; perPage?: number }> => {
        ...
        for (const u of listUrls) {
          const res = await fetchWithTimeout(u, { headers, timeoutMs: 25000, publicOnly: true });
          if (res.ok) {
